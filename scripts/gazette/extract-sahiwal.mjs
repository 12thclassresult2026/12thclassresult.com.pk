...
async function main() {
  ...
  for (let pageNum = 22; pageNum <= doc.numPages; pageNum++) {
    ...
    if (pageNum % 250 === 0 || pageNum === doc.numPages) {
      ...
      console.log(
        `[Sahiwal] Processed page ${pageNum}/${doc.numPages} (${totalCandidates} candidates, ${elapsed}s)`,
      )
    }
  }
  ...
  console.log(
    `[Sahiwal] FINISHED! Total: ${totalCandidates} candidates across ${fileIndex} SQL files in ${totalTime}s.`,
  )
