...
async function main() {
  ...
  for (let pageNum = 130; pageNum <= doc.numPages; pageNum++) {
    ...
    if (pageNum % 250 === 0 || pageNum === doc.numPages) {
      ...
      console.log(
        `[FBISE] Processed page ${pageNum}/${doc.numPages} (${totalCandidates} candidates, ${elapsed}s)`,
      )
    }
  }
  ...
  console.log(
    `[FBISE] FINISHED! Total: ${totalCandidates} candidates across ${fileIndex} SQL files in ${totalTime}s.`,
  )
