...
async function main() {
  ...
  for (let pageNum = 88; pageNum <= doc.numPages; pageNum++) {
    ...
    for (let c = 0; c < 2; c++) {
      ...
      for (let r = 0; r < rolls.length; r++) {
        ...
        const rowItems = body.filter(
          (i) => Math.abs(i.y - roll.y) <= 4 && i !== roll && (c === 0 ? i.x < 280 : i.x >= 280),
        )
    ...
    if (pageNum % 100 === 0 || pageNum === doc.numPages) {
      ...
      console.log(
        `[Bahawalpur] Processed page ${pageNum}/${doc.numPages} (${totalCandidates} candidates, ${elapsed}s)`,
      )
    }
  }
  ...
  console.log(
    `[Bahawalpur] FINISHED! Total: ${totalCandidates} candidates across ${fileIndex} SQL files in ${totalTime}s.`,
  )
