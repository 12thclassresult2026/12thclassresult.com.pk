...
async function main() {
  ...
  for (let pageNum = 116; pageNum <= doc.numPages; pageNum++) {
    ...
    if (pageNum % 100 === 0 || pageNum === doc.numPages) {
      ...
      console.log(
        `[Sargodha] Processed page ${pageNum}/${doc.numPages} (${totalCandidates} candidates, ${elapsed}s)`,
      )
    }
  }
  ...
  console.log(
    `[Sargodha] FINISHED! Total: ${totalCandidates} candidates across ${fileIndex} SQL files in ${totalTime}s.`,
  )
