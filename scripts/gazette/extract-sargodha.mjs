/**
 * BISE Sargodha 2025 HSSC-II Gazette PDF -> D1 SQL batches
 * 
 * Usage: node scripts/gazette/extract-sargodha.mjs
 */
import { createWriteStream, mkdirSync } from 'node:fs';
import { readFile } from 'node:fs/promises';

const PDF_PATH = 'gazette/2025-annual-12th/bise-sargodha_12th_2025-annual.pdf';
const OUT_DIR = 'gazette-private/d1-sql-sargodha';
const BOARD_ID = 'bise-sargodha';
const YEAR = 2025;
...
async function main() {
  console.log(`[Sargodha] Loading PDF: ${PDF_PATH}`);
  const pdfjs = await import('pdfjs-dist/legacy/build/pdf.mjs');
  const data = new Uint8Array(await readFile(PDF_PATH));
  const loadingTask = pdfjs.getDocument({ data, useSystemFonts: true });
  const doc = await loadingTask.promise;
  console.log(`[Sargodha] Total Pages: ${doc.numPages}`);
  ...
  for (let pageNum = 116; pageNum <= doc.numPages; pageNum++) {
    ...
    // In Sargodha: 3 columns along X:
    // Col 0: x < 280
    // Col 1: 280 <= x < 530
    // Col 2: x >= 530
    const body = items.filter((i) => i.y < 540);

    const cols = [
      {
        rolls: body.filter((i) => i.x < 100 && /^\d{6}$/.test(i.str)),
        nameMaxX: 215,
        colMaxX: 280,
      },
      {
        rolls: body.filter((i) => i.x >= 280 && i.x < 360 && /^\d{6}$/.test(i.str)),
        nameMaxX: 470,
        colMaxX: 530,
      },
      {
        rolls: body.filter((i) => i.x >= 530 && i.x < 610 && /^\d{6}$/.test(i.str)),
        nameMaxX: 720,
        colMaxX: 9999,
      },
    ];
      ...
      for (let r = 0; r < rolls.length; r++) {
        ...
        const rowItems = body.filter(
          (i) =>
            ...
            i !== roll &&
            (c === 0 ? i.x < 280 : c === 1 ? i.x >= 280 && i.x < 530 : i.x >= 530),
        );
    ...
    if (pageNum % 100 === 0 || pageNum === doc.numPages) {
      ...
      console.log(`[Sargodha] Processed page ${pageNum}/${doc.numPages} (${totalCandidates} candidates, ${elapsed}s)`);
    }
  }
  ...
  const datasetSql = `INSERT OR REPLACE INTO dataset VALUES (
    'bise-sargodha-2025-first-annual-v1',
    '${BOARD_ID}',
    ${YEAR},
    '${EXAM}',
    'active',
    'bise-sargodha-pdf',
    'https://bisesargodha.edu.pk',
    'sgd-three-column-1.0.0',
  ...
  console.log(`[Sargodha] FINISHED! Total: ${totalCandidates} candidates across ${fileIndex} SQL files in ${totalTime}s.`);

  await loadingTask.destroy();
}

main().catch((err) => {
  console.error('[Sargodha] Extraction error:', err);
  process.exit(1);
});
