...
async function main() {
  ...
  for (let pageNum = 54; pageNum <= doc.numPages; pageNum++) {
    ...
    if (pageNum % 100 === 0 || pageNum === doc.numPages) {
      ...
      console.log(
        `[Lahore] Processed page ${pageNum}/${doc.numPages} (${totalCandidates} candidates, ${elapsed}s)`,
      )
    }
  }
  ...
  console.log(
    `[Lahore] FINISHED! Total: ${totalCandidates} candidates across ${fileIndex} SQL files in ${totalTime}s.`,
  )
