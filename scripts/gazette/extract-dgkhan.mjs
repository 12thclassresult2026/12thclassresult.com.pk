...
async function main() {
  ...
  for (let pageNum = 70; pageNum <= doc.numPages; pageNum++) {
    ...
    if (pageNum % 250 === 0 || pageNum === doc.numPages) {
      ...
      console.log(
        `[DG Khan] Processed page ${pageNum}/${doc.numPages} (${totalCandidates} candidates, ${elapsed}s)`,
      )
    }
  }
  ...
  console.log(
    `[DG Khan] FINISHED! Total: ${totalCandidates} candidates across ${fileIndex} SQL files in ${totalTime}s.`,
  )
