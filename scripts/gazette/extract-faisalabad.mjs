...
async function main() {
  ...
  for (let pageNum = 170; pageNum <= doc.numPages; pageNum++) {
    ...
    if (pageNum % 250 === 0 || pageNum === doc.numPages) {
      ...
      console.log(
        `[Faisalabad] Processed page ${pageNum}/${doc.numPages} (${totalCandidates} candidates, ${elapsed}s)`,
      )
    }
  }
  ...
  console.log(
    `[Faisalabad] FINISHED! Total: ${totalCandidates} candidates across ${fileIndex} SQL files in ${totalTime}s.`,
  )
